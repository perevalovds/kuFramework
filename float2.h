...
public:
	...
	float2();
	explicit float2( float _scalar );
	float2( float _x, float _y );
    float2( const float3& vec );
    ...
    bool operator!=( const float2& vec ) const;
    bool match( const float2& vec, float tolerance=0.0001 ) const;

	/**
	 * Checks if vectors look in the same direction.
	 * Tolerance is specified in degree.
	 */
    bool isAligned( const float2& vec, float tolerance=0.0001 ) const;
    bool isAlignedRad( const float2& vec, float tolerance=0.0001 ) const;
    bool align( const float2& vec, float tolerance=0.0001 ) const;
    bool alignRad( const float2& vec, float tolerance=0.0001 ) const;
	...
	friend ostream& operator<<(ostream& os, const float2& vec);
	friend istream& operator>>(istream& is, const float2& vec);
...
//	OF_DEPRECATED_MSG("Use float2::lengthSquared() instead.", float squareLength() const);
...
inline float2::float2( float _scalar ): x(_scalar), y(_scalar) {};
inline float2::float2( float _x, float _y ):x(_x), y(_y) {}

// Getters and Setters.
//
//
inline void float2::set( float _scalar ) {
	x = _scalar;
	y = _scalar;
}
...
inline bool float2::match( const float2& vec, float tolerance ) const {
 ...
 * Tolerance is specified in degree.
 */
inline bool float2::isAligned( const float2& vec, float tolerance ) const { 
	return  fabs( this->angle( vec ) ) < tolerance;
}
inline bool float2::align( const float2& vec, float tolerance ) const {
    return isAligned( vec, tolerance );
}

inline bool float2::isAlignedRad( const float2& vec, float tolerance ) const {
	return  fabs( this->angleRad( vec ) ) < tolerance;
}
inline bool float2::alignRad( const float2& vec, float tolerance ) const {
...
inline ostream& operator<<(ostream& os, const float2& vec) {
	os << vec.x << ", " << vec.y;
	return os;
}
